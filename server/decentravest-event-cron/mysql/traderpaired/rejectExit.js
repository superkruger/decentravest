...
const create = async (event) => {
  ...
  let resp = await client.query('INSERT INTO event_traderpaired_rejectexit \
    (id, blockNumber, txHash, investmentId, wallet, trader, rejectFrom, value, eventDate) \
    VALUES(?,?,?,?,?,?,?,?,?)', 
    [
      event.id,
      event.blockNumber, 
...
const update = async (event) => {
  ...
  let resp = await client.query('UPDATE event_traderpaired_rejectexit \
    set blockNumber = ?, txHash = ?, investmentId = ?, wallet = ?, trader = ?, rejectFrom = ?, value = ?, eventDate = ? \
    WHERE id = ?', 
    [
      event.blockNumber, 
      event.transactionHash,
